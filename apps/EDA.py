...
layout = html.Div([
    dbc.Container([
        ...
        dbc.Row([
            ...
            dbc.Col(html.H5(children='Daily figures since 31 Dec 2019', className="text-center"), width=8,
                    className="mt-4"),
        ]),
        ...
        dbc.Row([
            dbc.Col(html.H5(children='Demographics', className="text-center"),
                    className="mt-4"),
        ]),
        ...
        dbc.Row([
            dbc.Col(html.H5(children='Vitals and Labs', className="text-center"),
                    className="mt-4"),
        ]),
...
def update_table_dm(demographics):
    if demographics == "ADMISSION_TYPE":
        df_dm = pd.crosstab(index=dm_y['ADMISSION_TYPE'], columns=dm_y['target'])
    elif demographics == "ADMISSION_LOCATION":
        df_dm = pd.crosstab(index=dm_y['ADMISSION_LOCATION'], columns=dm_y['target'])
    elif demographics == "INSURANCE":
        df_dm = pd.crosstab(index=dm_y['INSURANCE'], columns=dm_y['target'])
    elif demographics == "RELIGION":
        df_dm = pd.crosstab(index=dm_y['RELIGION'], columns=dm_y['target'])
    elif demographics == "GENDER":
        df_dm = pd.crosstab(index=dm_y['GENDER'], columns=dm_y['target'])
    elif demographics == "ETHNICITY":
        df_dm = pd.crosstab(index=dm_y['ETHNICITY'], columns=dm_y['target'])
    elif demographics == "DIAGNOSIS":
        df_dm = pd.crosstab(index=dm_y['DIAGNOSIS'], columns=dm_y['target'])
    ...
    fig2 = px.histogram(dm_y, y=demographics, color="target", template="simple_white",
                        labels={"target": ""})
...
@app.callback(Output('table_vital', 'figure'),
              Input('vitals', 'value'))
def update_table_vital(vitals):
    vital_columns = vital_y.columns
    temp = vital_y.groupby('target').describe()
    for i in range(0, 47):
        if vitals == vital_columns[i]:
