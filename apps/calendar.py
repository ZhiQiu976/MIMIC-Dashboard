...
layout = html.Div([
    dbc.Container([
        ...
        dbc.Row([
            dbc.Col(html.H6(children='Colored square represents predicted discharges to the selected location.'),
                    className="mb-4")
        ]),
        dbc.Row([
...
months = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'Novemver',
          'December']
...
def update_graph(year, dischloc, month):
    ...
    fig = go.Figure(data=ff.create_annotated_heatmap(z, x=x, y=y, text=textinfo, hoverinfo='text', annotation_text=ann,
